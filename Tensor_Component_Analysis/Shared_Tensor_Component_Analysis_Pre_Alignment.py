...
def align_sessions(session_condition_tensor_list, number_of_factors):

    average_tensors_all_conditions = []

    # Get Average Responses for Each Condition For Each Session
    number_of_conditions = len(session_condition_tensor_list[0])
    number_of_sessions = len(session_condition_tensor_list)
    ...
    for condition_index in range(number_of_conditions):
        ...
        for session_index in range(number_of_sessions):
            tensor = session_condition_tensor_list[session_index][condition_index]
            print("Tensor Shape", np.shape(tensor))
            average_tensor = np.mean(tensor, axis=2)
            average_tensor = np.transpose(average_tensor)
    ...
    for session_index in range(number_of_sessions):
        ...
        for item in condition_tensor_list:
            print(np.shape(item))
        stacked_tensor = np.vstack(condition_tensor_list)
        print("Stacked Tensor Shape", np.shape(stacked_tensor))
    ...
    for session in range(number_of_sessions):
        ...
        regression_coefficients = tf.convert_to_tensor(regression_coefficients, dtype=tf.float32)
...
def train_model(trial_tensors_list, alignment_matricies, number_of_factors):
    ...
    for session in range(number_of_sessions):
        ...
        model.neuron_weights = alignmnet_matricies[session]
...
def extract_tensors(file_list, trial_start, trial_stop):
    ...
    for matlab_file_location in file_list:
        ...
        # Populate Session Condition Tensor List For Later Alignmnet
        visual_context_tensor = create_trial_tensor(delta_f_matrix, visual_context_onsets, trial_start, trial_stop)
        odour_context_tensor = create_trial_tensor(delta_f_matrix, odour_context_onsets, trial_start, trial_stop)
        condition_tensor_list = [visual_context_tensor, odour_context_tensor]
        session_condition_tensor_list.append(condition_tensor_list)

    return tensor_list, blocks_list, switches_list, switch_classification_list, session_name_list, session_condition_tensor_list
...
trial_stop = 40
number_of_factors = 7
tensor_list, blocks_list, switches_list, switch_classification_list, session_name_list, session_condition_tensor_list = extract_tensors(file_list, trial_start=trial_start, trial_stop=trial_stop)

# Get Initial Alignment Matricies
alignmnet_matricies = align_sessions(session_condition_tensor_list, number_of_factors)


# Train Model
model_list, shared_trial_weights = train_model(tensor_list, alignmnet_matricies, number_of_factors)
