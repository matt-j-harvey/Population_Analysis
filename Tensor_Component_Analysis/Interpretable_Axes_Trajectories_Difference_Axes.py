...
def perform_interpretable_axis(file_list, trial_start=-10, trial_stop=30, offset_onset=1):
    ...
    for matlab_file_location in file_list:
        ...
        # Extract Trials
        visual_context_vis_1_onsets = data_object.vis1_frames[offset_onset]
        visual_context_vis_2_onsets = data_object.vis2_frames[offset_onset]
        odour_context_vis_1_onsets = data_object.irrel_vis1_frames[offset_onset]
        odour_context_vis_2_onsets = data_object.irrel_vis2_frames[offset_onset]
        ...
        context_vector = np.subtract(mean_visual_trial_response, mean_odour_trial_response)

        # Normalise Context Vector
        context_vector = np.divide(context_vector, np.linalg.norm(context_vector))

        # Get Discrimination Vector
        vis_1_tensor = create_trial_tensor(delta_f_matrix, visual_context_vis_1_onsets, trial_start, trial_stop)
        vis_2_tensor = create_trial_tensor(delta_f_matrix, visual_context_vis_2_onsets, trial_start, trial_stop)

        # Take Mean Across Trials
        mean_vis_1_response = np.mean(vis_1_tensor, axis=0)
        mean_vis_2_response = np.mean(vis_2_tensor, axis=0)
        ...
        # Normalise Discrimination Vector
        discrimination_vector = np.divide(discrimination_vector, np.linalg.norm(discrimination_vector))



        # Extract Switch Trials
        expected_odour_trials = data_object.mismatch_trials['exp_odour'][offset_onset]
        ...
        for switch in range(number_of_switches):
            ...
            for timepoint in range(trial_start, trial_stop):
                ...
                coords = [context_projection, discrimination_projection]
