...
def get_model_trajectories(model, data, batch_size):
    ...
    # Else Give THe Network A Padded Input Data
        ...
        translated_input = model.timeseries_activation_layer([batch_data, model.translation_input_weights])
        initial_positions = model.encoder(translated_input)[2]
        trajectories = model.generator(initial_positions)
...
def train_lfads_single_session(base_directory, save_directory, conditions, trial_start, trial_stop):
    ...
    # Load data
    condition_1_tensor, condition_2_tensor, full_tensor = load_data(base_directory, conditions, trial_start, trial_stop)
    ...
    batch_size = tensor_1_samples + tensor_2_samples

    # Create Model
    model = LFADS_Model.LFADS_Model(number_of_neurons, number_of_timepoints, latent_dimensions, number_of_factors, 'session_' + str(0), batch_size)
    ...
    for epoch in range(1000):
        ...
        model.fit(batch_data, epochs=epoch_step, batch_size=batch_size, verbose=1)

        # Save Weights
        model.save_weights(weights_save_directory, save_format='h5')

        plot_all_trajectories([model], weights_save_directory, [condition_1_tensor], [condition_2_tensor], plot_directory, batch_size, epoch)
...
train_lfads_single_session(file_list[0], save_directory, conditions, trial_start, trial_stop)