...
def get_model_trajectories(model, data):

    translated_input = model.timeseries_activation_layer([data, model.translation_weights])
...
def train_lfads_model(file_list, save_directory, conditions, trial_start, trial_stop):
    ...
    model_list = []
    for session_index in range(number_of_sessions):
        model = LFADS_Model.LFADS_Model(neuron_size_list[session_index], number_of_timepoints, 40, number_of_factors, 'session_' + str(session_index), batch_size)
    ...
    for epoch in range(1000):
        ...
        for session_index in range(number_of_sessions):
            ...
            # Get Training Data
            session_indexes = session_indexes_list[session_index]
            random.shuffle(session_indexes)
            sample_trials = session_indexes[0:batch_size]
            sample_data = data_list[session_index][sample_trials]

            # Fit Model

            print("Model Weights Shape", np.shape(model.translation_weights))
            print("Sample Data Shape", np.shape(sample_data))
            print("Batch Size", batch_size)
            model.fit(sample_data, epochs=epoch_step, batch_size=batch_size, callbacks=[LearningRateScheduler(learning_rate_function, verbose=1)])

            # Save Weights
            model.save_weights(weights_save_directory, save_format='h5')

            # Get Trajectories
            condition_1_tensor = condition_1_tensor_list[session_index]
            condition_2_tensor = condition_2_tensor_list[session_index]

            condition_1_trajectories = get_model_trajectories(model, condition_1_tensor[session_index:batch_size])
            condition_2_trajectories = get_model_trajectories(model, condition_2_tensor[session_index:batch_size])
